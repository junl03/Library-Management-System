...
int main()
{
    ...
    do
    {
        ...
        do
        {
            ...
            user_match = user_database.searchByKey(user_database.root, username);  //get corresponding node to username

            // login fail
            if (user_match == NULL)
            {
                cerr << "Incorrect username or password!" << endl << endl;
                continue;
            }
            else if (user_match->data->getPassword() != password)
            {
                cerr << "Incorrect username or password!" << endl << endl;
                continue;
            }
        ...
        if (class_name == "Student")
        {
            ...
            {
                if (user_vect[i]->getUsername() == user_match->data->getUsername())
                {
            ...
            if (new_user == 1)
            {
                student_temp = new Student(user_match->data->getUsername(), user_match->data->getPassword());
            }
            ...
            if (new_user == 1)
            {
                user_vect.push_back(student_temp);
            }
        ...
        else if (class_name == "Teacher")
        {
            ...
            if (new_user == 1)
            {
                teacher_temp = new Teacher(user_match->data->getUsername(), user_match->data->getPassword());
            }
            ...
            do {
                ...
                case 2:
                    ...
                    user_database.updateTreeByData(user_database.root, teacher_temp);
                    break;
                case 3:
            ...
            if (new_user == 1)
            {
                user_vect.push_back(teacher_temp);
            }
        ...
        else if (class_name == "Librarian")
        {
            ...
            if (new_user == 1)
            {
                librarian_temp = new Librarian(user_match->data->getUsername(), user_match->data->getPassword());
            }
        ...
        for (int i = 0; i < user_vect.size(); i++)
        {
            user_match = user_database.searchByKey(user_database.root, user_vect[i]->getUsername());  //get corresponding node to username
            ...
            // student type
            if (user_match->data->getClassName() == "Student")
            {
                student_temp = dynamic_cast<Student*>(user_match->data);
                // update borrowed copies
                vector<Copy> borrowed_copies = student_temp->getBorrowedCopies();
                for (int j = 0; j < borrowed_copies.size(); j++)
                {
                    borrowed_copies[i].setBorrowStartDate(sDate);
                    copy_database.updateTreeByData(copy_database.root, borrowed_copies[i]);
;               }
                student_temp->setBorrowedCopies(borrowed_copies);
                // update reserved copies
                vector<Copy> reserved_copies = student_temp->getReservedCopies();
                for (int j = 0; j < reserved_copies.size(); j++)
                {
                    reserved_copies[i].setReserveStartDate(sDate); 
                }
                student_temp->setReservedCopies(reserved_copies);
                // update user database
                user_database.updateTreeByData(user_database.root, student_temp);
            }

            // teacher type
            else if (user_match->data->getClassName() == "Teacher")
            {
                teacher_temp = dynamic_cast<Teacher*>(user_match->data);
                cout << teacher_temp;
                // update borrowed copies
                vector<Copy> borrowed_copies = teacher_temp->getBorrowedCopies();
                for (int j = 0; j < borrowed_copies.size(); j++)
                {
                    cout << "Borrowed copies" << endl;     
                    borrowed_copies[i].setBorrowStartDate(sDate);
                    cout << borrowed_copies[i] << endl; //test
                    TreeNode<Copy>* copy_node = copy_database.searchByKey(copy_database.root, borrowed_copies[i].getID());
                    copy_node->data = borrowed_copies[i];
                    copy_database.updateTreeByNode(copy_database.root, copy_node);
                }
                teacher_temp->setBorrowedCopies(borrowed_copies);
                // update reserved copies
                vector<Copy> reserved_copies = teacher_temp->getReservedCopies();
                for (int j = 0; j < reserved_copies.size(); j++)
                {
                    reserved_copies[i].setReserveStartDate(sDate);
                }
                teacher_temp->setReservedCopies(reserved_copies);
                // update user database
                user_database.updateTreeByData(user_database.root, teacher_temp);
            }
...
void writeToUserFile(TreeNode<User*>* root, std::ofstream& f)
{
    ...
    if (node)
    {
        ...
        if (name == "Student")   // compare whether user type of current node is student
        {
            f << 0 << "\t" << node->data->getUsername() << "\t" << node->data->getPassword() << endl;
        }
        else if (name == "Teacher")  // compare whether user type of current node is teacher
        {
